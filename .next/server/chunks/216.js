...
var react_spring_shared_modern_raf = (fn) => schedule(fn, updateQueue);
var writeQueue = makeQueue();
react_spring_shared_modern_raf.write = (fn) => schedule(fn, writeQueue);
var onStartQueue = makeQueue();
react_spring_shared_modern_raf.onStart = (fn) => schedule(fn, onStartQueue);
var onFrameQueue = makeQueue();
react_spring_shared_modern_raf.onFrame = (fn) => schedule(fn, onFrameQueue);
var onFinishQueue = makeQueue();
react_spring_shared_modern_raf.onFinish = (fn) => schedule(fn, onFinishQueue);
var timeouts = [];
react_spring_shared_modern_raf.setTimeout = (handler, ms) => {
  const time = react_spring_shared_modern_raf.now() + ms;
  const cancel = () => {
...
react_spring_shared_modern_raf.cancel = (fn) => {
...
react_spring_shared_modern_raf.sync = (fn) => {
  sync = true;
  react_spring_shared_modern_raf.batchedUpdates(fn);
  sync = false;
};
react_spring_shared_modern_raf.throttle = (fn) => {
  let lastArgs;
  function queuedFn() {
    try {
      fn(...lastArgs);
  ...
  function throttled(...args) {
    lastArgs = args;
    react_spring_shared_modern_raf.onStart(queuedFn);
  }
...
react_spring_shared_modern_raf.use = (impl) => nativeRaf = impl;
react_spring_shared_modern_raf.now = typeof performance != "undefined" ? () => performance.now() : Date.now;
react_spring_shared_modern_raf.batchedUpdates = (fn) => fn();
react_spring_shared_modern_raf.catch = console.error;
react_spring_shared_modern_raf.frameLoop = "always";
react_spring_shared_modern_raf.advance = () => {
  if (react_spring_shared_modern_raf.frameLoop !== "demand") {
    console.warn(
...
function loop() {
  if (~ts) {
    nativeRaf(loop);
    react_spring_shared_modern_raf.batchedUpdates(update);
  }
}
function update() {
  const prevTs = ts;
  ts = react_spring_shared_modern_raf.now();
...
function eachSafely(values, each2) {
  values.forEach((value) => {
    try {
      ...
      react_spring_shared_modern_raf.catch(e);
    }
  });
}
...
function react_spring_shared_modern_isEqual(a, b) {
...
function react_spring_shared_modern_eachProp(obj, fn, ctx) {
...
var react_spring_shared_modern_toArray = (a) => react_spring_shared_modern_is.und(a) ? [] : react_spring_shared_modern_is.arr(a) ? a : [a];
function react_spring_shared_modern_flush(queue, iterator) {
  if (queue.size) {
...
var react_spring_shared_modern_flushCalls = (queue, ...args) => react_spring_shared_modern_flush(queue, (fn) => fn(...args));
...
var react_spring_shared_modern_assign = (globals) => {
  if (globals.to)
    to = globals.to;
  if (globals.now)
    react_spring_shared_modern_raf.now = globals.now;
  ...
  if (globals.requestAnimationFrame)
    react_spring_shared_modern_raf.use(globals.requestAnimationFrame);
  if (globals.batchedUpdates)
    react_spring_shared_modern_raf.batchedUpdates = globals.batchedUpdates;
  if (globals.willAdvance)
    willAdvance = globals.willAdvance;
  if (globals.frameLoop)
    react_spring_shared_modern_raf.frameLoop = globals.frameLoop;
};
...
var frameLoop = {
  ...
  start(animation) {
    if (priority > animation.priority) {
      ...
      react_spring_shared_modern_raf.onStart(flushStartQueue);
    } else {
  ...
  sort(animation) {
    if (priority) {
      react_spring_shared_modern_raf.onFrame(() => frameLoop.sort(animation));
    } else {
...
var react_spring_shared_modern_hasFluidValue = (arg) => Boolean(arg && arg[$get]);
var react_spring_shared_modern_getFluidValue = (arg) => arg && arg[$get] ? arg[$get]() : arg;
var react_spring_shared_modern_getFluidObservers = (target) => target[$observers] || null;
...
function react_spring_shared_modern_addFluidObserver(target, observer2) {
  if (target[$get]) {
...
function react_spring_shared_modern_removeFluidObserver(target, observer2) {
  const observers = target[$observers];
...
var createStringInterpolator2 = (config) => {
  ...
  const output = config.output.map((value) => {
    return react_spring_shared_modern_getFluidValue(value).replace(cssVariableRegex, variableToRgba).replace(colorRegex, colorToRgba).replace(namedColorRegex, colorToRgba);
  });
...
function react_spring_shared_modern_isAnimatedString(value) {
...
var react_spring_shared_modern_onScroll = (callback, { container = document.documentElement } = {}) => {
  ...
  react_spring_shared_modern_raf(animateScroll);
  return () => {
    react_spring_shared_modern_raf.cancel(animateScroll);
...
var react_spring_shared_modern_useIsomorphicLayoutEffect = isSSR() ? react_.useEffect : react_.useLayoutEffect;
...
function react_spring_shared_modern_useForceUpdate() {
  const update2 = (0,react_.useState)()[1];
...
function useMemoOne(getResult, inputs) {
  const [initial] = (0,react_.useState)(
    () => ({
      inputs,
      result: getResult()
    })
  );
  const committed = (0,react_.useRef)();
...
var react_spring_shared_modern_useOnce = (effect) => (0,react_.useEffect)(effect, emptyDeps);
var emptyDeps = [];
...
var react_spring_animated_modern_setAnimated = (owner, node) => defineHidden(owner, $node, node);
var react_spring_animated_modern_getPayload = (owner) => owner && owner[$node] && owner[$node].getPayload();
var Animated = class {
  constructor() {
    react_spring_animated_modern_setAnimated(this, this);
  }
...
var react_spring_animated_modern_AnimatedValue = class extends Animated {
  constructor(_value) {
    super();
    this._value = _value;
    this.done = true;
  ...
  /** @internal */
  static create(value) {
    return new react_spring_animated_modern_AnimatedValue(value);
  }
  getPayload() {
    return [this];
  }
...
var react_spring_animated_modern_AnimatedString = class extends react_spring_animated_modern_AnimatedValue {
  constructor(value) {
    super(0);
    this._string = null;
  ...
  /** @internal */
  static create(value) {
    return new react_spring_animated_modern_AnimatedString(value);
  }
  getValue() {
...
var AnimatedObject = class extends Animated {
  ...
  getValue(animated) {
    const values = {};
    react_spring_shared_modern_eachProp(this.source, (source, key) => {
      if (isAnimated(source)) {
        values[key] = source.getValue(animated);
      } else if (react_spring_shared_modern_hasFluidValue(source)) {
        values[key] = react_spring_shared_modern_getFluidValue(source);
      } else if (!animated) {
        values[key] = source;
      }
    });
  ...
  _makePayload(source) {
    if (source) {
      ...
      react_spring_shared_modern_eachProp(source, this._addToPayload, payload);
      return Array.from(payload);
    }
  }
  /** Add to a payload set. */
  _addToPayload(source) {
    if (TreeContext.dependencies && react_spring_shared_modern_hasFluidValue(source)) {
      TreeContext.dependencies.add(source);
    }
    const payload = react_spring_animated_modern_getPayload(source);
    if (payload) {
...
function makeAnimated(value) {
  const nodeType = react_spring_shared_modern_isAnimatedString(value) ? react_spring_animated_modern_AnimatedString : react_spring_animated_modern_AnimatedValue;
  return nodeType.create(value);
}

// src/getAnimatedType.ts

function react_spring_animated_modern_getAnimatedType(value) {
  const parentNode = getAnimated(value);
  return parentNode ? parentNode.constructor : react_spring_shared_modern_is.arr(value) ? AnimatedArray : react_spring_shared_modern_isAnimatedString(value) ? react_spring_animated_modern_AnimatedString : react_spring_animated_modern_AnimatedValue;
}
...
var withAnimated = (Component, host) => {
  ...
  return (0,react_.forwardRef)((givenProps, givenRef) => {
    const instanceRef = (0,react_.useRef)(null);
    const ref = hasInstance && // eslint-disable-next-line react-hooks/rules-of-hooks
    (0,react_.useCallback)(
      (value) => {
    ...
    const observerRef = (0,react_.useRef)();
    react_spring_shared_modern_useIsomorphicLayoutEffect(() => {
      observerRef.current = observer;
      react_spring_shared_modern_each(deps, (dep) => react_spring_shared_modern_addFluidObserver(dep, observer));
      return () => {
        if (observerRef.current) {
          react_spring_shared_modern_each(
            observerRef.current.deps,
            (dep) => react_spring_shared_modern_removeFluidObserver(dep, observerRef.current)
          );
          react_spring_shared_modern_raf.cancel(observerRef.current.update);
        }
      };
    });
    (0,react_.useEffect)(callback, []);
    react_spring_shared_modern_useOnce(() => () => {
      const observer2 = observerRef.current;
      react_spring_shared_modern_each(observer2.deps, (dep) => react_spring_shared_modern_removeFluidObserver(dep, observer2));
    });
    const usedProps = host.getComponentProps(props.getValue());
    return /* @__PURE__ */ react_.createElement(Component, { ...usedProps, ref });
  });
};
var PropsObserver = class {
  ...
  eventObserved(event) {
    if (event.type == "change") {
      react_spring_shared_modern_raf.write(this.update);
    }
  }
};
...
var createHost = (components, {
  ...
  react_spring_shared_modern_eachProp(components, (Component, key) => {
...
var DEFAULT_PROPS = (/* unused pure expression or super */ null && ([
  "config",
  "onProps",
  "onStart",
  "onChange",
...
// src/Animation.ts
var emptyArray = (/* unused pure expression or super */ null && ([]));
var Animation = class {
  constructor() {
    this.changed = false;
...
var BailSignal = class extends (/* unused pure expression or super */ null && (Error)) {
  constructor() {
    super(
...
var SkipAnimationSignal = class extends (/* unused pure expression or super */ null && (Error)) {
  constructor() {
...
// src/SpringValue.ts
var SpringValue = class extends (/* unused pure expression or super */ null && (FrameValue)) {
...
var ACTIVE_EVENTS = (/* unused pure expression or super */ null && ([
  "onStart",
  "onRest",
  "onChange",
  "onPause",
...
var BATCHED_EVENTS = (/* unused pure expression or super */ null && (["onStart", "onChange", "onRest"]));
var nextId2 = 1;
...
var SpringContext = ({
  ...
  const inherited = (0,react_.useContext)(ctx);
  const pause = props.pause || !!inherited.pause, immediate = props.immediate || !!inherited.immediate;
  props = useMemoOne(() => ({ pause, immediate }), [pause, immediate]);
  const { Provider } = ctx;
  return /* @__PURE__ */ react_.createElement(Provider, { value: props }, children);
};
var ctx = makeContext(SpringContext, {});
SpringContext.Provider = ctx.Provider;
SpringContext.Consumer = ctx.Consumer;
function makeContext(target, init) {
  Object.assign(target, react_.createContext(init));
...
var Interpolation = class extends FrameValue {
  constructor(source, args) {
    ...
    const nodeType = react_spring_animated_modern_getAnimatedType(value);
    react_spring_animated_modern_setAnimated(this, nodeType.create(value));
  }
  advance(_dt) {
    const value = this._get();
    const oldValue = this.get();
    if (!react_spring_shared_modern_isEqual(value, oldValue)) {
  ...
  _get() {
    const inputs = react_spring_shared_modern_is.arr(this.source) ? this.source.map(react_spring_shared_modern_getFluidValue) : react_spring_shared_modern_toArray(react_spring_shared_modern_getFluidValue(this.source));
    return this.calc(...inputs);
  }
  _start() {
    if (this.idle && !checkIdle(this._active)) {
      this.idle = false;
      react_spring_shared_modern_each(react_spring_animated_modern_getPayload(this), (node) => {
        node.done = false;
      });
      if (globals_exports.skipAnimation) {
        react_spring_shared_modern_raf.batchedUpdates(() => this.advance());
        becomeIdle(this);
      } else {
  ...
  _attach() {
    let priority = 1;
    react_spring_shared_modern_each(react_spring_shared_modern_toArray(this.source), (source) => {
      if (react_spring_shared_modern_hasFluidValue(source)) {
        react_spring_shared_modern_addFluidObserver(source, this);
      }
  ...
  _detach() {
    react_spring_shared_modern_each(react_spring_shared_modern_toArray(this.source), (source) => {
      if (react_spring_shared_modern_hasFluidValue(source)) {
        react_spring_shared_modern_removeFluidObserver(source, this);
      }
    });
    this._active.clear();
    becomeIdle(this);
  ...
  eventObserved(event) {
    if (event.type == "change") {
      ...
      this.priority = react_spring_shared_modern_toArray(this.source).reduce(
...
function becomeIdle(self) {
  if (!self.idle) {
    self.idle = true;
    react_spring_shared_modern_each(react_spring_animated_modern_getPayload(self), (node) => {
      node.done = true;
    });
...
var AnimatedStyle = class extends AnimatedObject {
  constructor({ x, y, z, ...style }) {
    ...
    react_spring_shared_modern_eachProp(style, (value, key) => {
      if (key === "transform") {
        inputs.push([value || ""]);
        ...
        inputs.push(react_spring_shared_modern_toArray(value));
        transforms.push(
...
var FluidTransform = class extends FluidValue {
  ...
  _get() {
    ...
    react_spring_shared_modern_each(this.inputs, (input, i) => {
      const arg1 = react_spring_shared_modern_getFluidValue(input[0]);
      const [t, id] = this.transforms[i](
        react_spring_shared_modern_is.arr(arg1) ? arg1 : input.map(react_spring_shared_modern_getFluidValue)
      );
      transform += " " + t;
    ...
    if (count == 1)
      react_spring_shared_modern_each(
        ...
        (input) => react_spring_shared_modern_each(
          input,
          (value) => react_spring_shared_modern_hasFluidValue(value) && react_spring_shared_modern_addFluidObserver(value, this)
        )
      );
  }
  ...
  observerRemoved(count) {
    if (count == 0)
      react_spring_shared_modern_each(
        ...
        (input) => react_spring_shared_modern_each(
          input,
          (value) => react_spring_shared_modern_hasFluidValue(value) && react_spring_shared_modern_removeFluidObserver(value, this)
        )
      );
  }
  eventObserved(event) {
...
globals_exports.assign({
  batchedUpdates: server_rendering_stub_.unstable_batchedUpdates,
  createStringInterpolator: createStringInterpolator2,
  colors: colors2
});
